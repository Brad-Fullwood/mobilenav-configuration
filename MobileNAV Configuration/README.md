...
| `Functions(CodeunitId, ServiceName)` | Gives a dialog page its own function codeunit, published as a web service under `ServiceName`. MobileNAV runs a dialog's buttons through a codeunit service (by default its own report-function codeunit, which knows nothing of your buttons — the tap fails with `Method "…" is invalid!`), so a dialog with a `Button` needs this, and each `Button` names its procedure with `FunctionName`. The procedure's parameters are filled from the dialog's controls of the same name; it returns MobileNAV's function result text (`"MobileNAV Result Helper".ConvertReturnValuesToResult`). Needs `PublishAsDialog` first. |
