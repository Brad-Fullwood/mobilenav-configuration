...
It extracts the general configuration mechanism first implemented in Translift's `Setup.Codeunit.al`: publish page web services, register drill-target pages, refresh host-page metadata, expose extension fields, and bind fields that open filtered MobileNAV pages. Consumer-specific page ids, control names, and filters stay in the consuming extension.
